...
{{
    ...
    "시각화_코드": {{
        "react_component": "성적 데이터를 시각화하는 React 컴포넌트 코드 (Chart.js 또는 Recharts 사용)",
        "css_styles": "시각화에 필요한 CSS 스타일 코드",
        "설명": "컴포넌트 사용 방법 및 필요한 라이브러리 설명"
    }}
}}

주의사항:
1. 모든 분석은 객관적인 데이터를 기반으로 해주세요.
2. 구체적이고 실행 가능한 제안을 해주세요.
3. 학생의 강점을 최대한 살리는 방향으로 분석해주세요.
4. 모든 값은 문자열 형태로 반환해주세요.
5. React 컴포넌트는 Chart.js 또는 Recharts를 사용하여 성적 데이터를 시각적으로 표현해주세요.
6. 시각화는 다음 요소들을 포함해야 합니다:
   - 과목별 석차등급 비교 차트
   - 원점수/과목평균 비교 차트
   - 성취도 분포 차트
   - 학기별 성적 추이 차트
